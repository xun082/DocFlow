...
// 调试：检查组件是否正确导入
console.log('ChatPanel imports debug:', {
...
export function ChatPanel({
  ...
  return (
    <div
      className={`flex flex-col flex-1 min-w-0 bg-gradient-to-br from-gray-50 to-purple-50/30 ${
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {QUICK_QUESTIONS.map((question) => (
            <button
              ...
              className="px-3 py-1.5 text-xs text-gray-600 bg-white border border-gray-200 rounded-full hover:bg-gray-50 hover:border-purple-200 transition-colors"
            >
              {question.text}
            </button>
          ))}
        ...
        <div className="relative">
          <Textarea
            ...
            placeholder="请输入提示词..."
            className="min-h-[80px] pr-12 resize-none bg-white border-gray-200 focus:border-purple-300 focus:ring-purple-200"
          />
          {/* 发送按钮 */}
          <Button
            size="icon"
            onClick={onSend}
            className="absolute right-2 bottom-2 h-8 w-8 bg-purple-600 hover:bg-purple-700"
          >
