...
import { DragOverlay, Modifier, defaultDropAnimation } from '@dnd-kit/core';
...
// function useFolderDroppable(dropOptions: UseDroppableArguments) {}
...
const FileTree: React.FC<FileTreeProps> = ({
  ...
  const activeFile = files.find(({ id }) => id === selectedFileId);
  ...
  const RenderFile: React.FC<{
    ...
    depth?: number;
    id: string;
  }> = ({ file, depth = 0, id }): React.ReactElement => {
    ...
    return (
      <div
        ...
        className={clsx(styles.Wrapper, isDragging && styles.indicator)}
      ...
      >
        <div {...listeners} {...attributes} style={style}>
          <div
            ...
            onMouseDown={(e) => onFileSelect(file, e)}
  ...
  return (
    <div className="py-2">
      <SortableContext strategy={verticalListSortingStrategy} items={fileSordId}>
        {files.map((file) => (
          <RenderFile key={file.id} file={file} depth={file.depth} id={file.id} />
        ))}
        {createPortal(
          <DragOverlay modifiers={[adjustTranslate]} dropAnimation={{ ...defaultDropAnimation }}>
            {activeFile && (
              <RenderFile depth={activeFile.depth} file={activeFile} id={activeFile.id} />
            )}
          </DragOverlay>,
          document.body,
        )}
