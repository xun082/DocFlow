...
const KnowledgeBaseSelector: React.FC<KnowledgeBaseSelectorProps> = ({
  ...
  // 获取知识库列表
  useEffect(() => {
    const fetchKnowledgeOptions = async () => {
      setIsLoading(true);
      ...
      try {
        ...
        // result.data 是 { data: KnowledgeOption[] }
        // result.data.data 是 KnowledgeOption[]
    ...
    if (buttonConfig.isActive && isOpen) {
      fetchKnowledgeOptions();
      setDisplayCount(INITIAL_DISPLAY_COUNT);
      setSearchQuery('');
    }
  }, [buttonConfig.isActive, isOpen]);
